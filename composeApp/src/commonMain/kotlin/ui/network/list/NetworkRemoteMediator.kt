...
class NetworkRemoteMediator (
    ...
    override suspend fun initialize(): InitializeAction {
        return if (Clock.System.now().toEpochMilliseconds() - (roomDatabase.pagingMetaDao().getCreationTime() ?: 0) < cacheTimeoutMillis) {
        ...
        try {
            ...
            return withContext(Dispatchers.IO) {
                if (loadType == LoadType.REFRESH) {
                    page = initialPage
                    roomDatabase.pagingMetaDao().removeAll()
                    roomDatabase.networkItemDbDao().removeAll()
                }
    ...
    private suspend fun getPagingMetaClosestToCurrentPosition(state: PagingState<Int, NetworkItemIO>): PagingMetaIO? {
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.userPublicId?.let { id ->
                roomDatabase.pagingMetaDao().getPagingMetaByEntityId("${currentUserUid}_$id")
            }
        }
    }
