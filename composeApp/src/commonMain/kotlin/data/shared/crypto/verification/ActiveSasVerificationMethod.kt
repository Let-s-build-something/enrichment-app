...
class ActiveSasVerificationMethod private constructor(
    override val startEventContent: VerificationStartEventContent.SasStartEventContent,
    ...
    private val _state: MutableStateFlow<ActiveSasVerificationState> =
        MutableStateFlow(
            if (weStartedVerification) ActiveSasVerificationState.OwnSasStart(
                startEventContent
            )
            else ActiveSasVerificationState.TheirSasStart(
                startEventContent,
                olmSas,
                json,
                relatesTo,
    ...
    companion object {
        ...
        suspend fun create(
            startEventContent: VerificationStartEventContent.SasStartEventContent,
            ...
            return when {
                startEventContent.keyAgreementProtocols.none { it == SasKeyAgreementProtocol.Curve25519HkdfSha256 } -> {
                    sendVerificationStep(
    ...
    override suspend fun handleVerificationStep(step: VerificationStep, isOurOwn: Boolean) {
        ...
        when (step) {
            is SasAcceptEventContent -> {
                if (currentState is ActiveSasVerificationState.OwnSasStart || currentState is ActiveSasVerificationState.TheirSasStart)
                    onAccept(step, isOurOwn)
                else cancelUnexpectedMessage(currentState)
            }

            is SasKeyEventContent -> {
                if (currentState is ActiveSasVerificationState.Accept || currentState is ActiveSasVerificationState.WaitForKeys)
                    onKey(step, isOurOwn)
                else cancelUnexpectedMessage(currentState)
            }

            is SasMacEventContent -> {
                if (currentState is ActiveSasVerificationState.ComparisonByUser || currentState is ActiveSasVerificationState.WaitForMacs)
                    onMac(step, isOurOwn)
                else cancelUnexpectedMessage(currentState)
            }
    ...
    private suspend fun cancelUnexpectedMessage(currentState: ActiveSasVerificationState) {
        sendVerificationStep(
            VerificationCancelEventContent(
                VerificationCancelEventContent.Code.UnexpectedMessage,
    ...
    private suspend fun onAccept(stepContent: SasAcceptEventContent, isOurOwn: Boolean) {
        _state.value =
            ActiveSasVerificationState.Accept(isOurOwn)
        ...
        if (!isOurOwn) {
            when {
                ...
                stepContent.keyAgreementProtocol != SasKeyAgreementProtocol.Curve25519HkdfSha256 ->
                    sendVerificationStep(
                        VerificationCancelEventContent(
                            VerificationCancelEventContent.Code.UnknownMethod,
                ...
                stepContent.messageAuthenticationCode != SasMessageAuthenticationCode.HkdfHmacSha256
                        && stepContent.messageAuthenticationCode != SasMessageAuthenticationCode.HkdfHmacSha256V2 ->
                    sendVerificationStep(
                        VerificationCancelEventContent(
                            VerificationCancelEventContent.Code.UnknownMethod,
    ...
    private suspend fun onKey(stepContent: SasKeyEventContent, isOurOwn: Boolean) {
        ...
        when (val currentState = state.value) {
            is ActiveSasVerificationState.Accept -> {
                _state.value =
                    ActiveSasVerificationState.WaitForKeys(
                        isOurOwn
                    )
            ...
            is ActiveSasVerificationState.WaitForKeys -> {
                if (currentState.isOurOwn != isOurOwn) {
                    fun createComparison() {
                        ...
                        _state.value = ActiveSasVerificationState.ComparisonByUser(
                    ...
                    if (!isOurOwn) {
                        if (theirCommitment != null
                            ...
                            VerificationCancelEventContent(
                                VerificationCancelEventContent.Code.MismatchedCommitment,
    ...
    private suspend fun onMac(stepContent: SasMacEventContent, isOurOwn: Boolean) {
        ...
        when {
            theirMac == null && state.value is ActiveSasVerificationState.ComparisonByUser -> _state.value =
                ActiveSasVerificationState.WaitForMacs
            theirMac != null && (state.value == ActiveSasVerificationState.WaitForMacs || isOurOwn) -> {
    ...
    private suspend fun checkHkdfHmacSha256Mac(theirMac: SasMacEventContent, calculateMac: (String, String) -> String) {
        ...
        if (keys == theirMac.keys) {
            val allKeysOfDevice = keyStore.getAllKeysFromUser<Key.Ed25519Key>(theirUserId, theirDeviceId)
            ...
            if (!containsMismatchedMac) {
                ...
                sendVerificationStep(
                    VerificationCancelEventContent(
                        VerificationCancelEventContent.Code.KeyMismatch,
                        "macs did not match",
                        relatesTo,
                        transactionId
                    )
...
internal suspend inline fun <reified T : Key> OlmCryptoStore.getAllKeysFromUser(
    userId: UserId,
    filterDeviceId: String? = null,
    filterUsage: CrossSigningKeysUsage? = null
): Set<T> {
    val deviceKeys = getDeviceKeys(userId.full).entries
        .filter { if (filterDeviceId != null) it.key == filterDeviceId else true }
        .flatMap { it.value.value.signed.keys.filterIsInstance<T>() }
